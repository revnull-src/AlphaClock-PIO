...
main.cpp (formerly AlphaClock.ino)

-- Alpha Clock Five Firmware, version 2.2.1 --

Version 2.2.1 - March 25, 2023
...
Environment: Re-designed to work with PlatformIO.
...
a typical upload speed of 57200 baud. Please select the appropriate
PlatformIO build environment based on your bootloader speed
requirements.
...
#include <alphafive.h> // Alpha Clock Five library, https://github.com/oskay/AlphaClock
...
void DisplayWord(String Word, unsigned int duration)
{
    // Usage: DisplayWord ("ALARM", 500);

    modeShowText = 1;
    const char *WordIn = Word.c_str();
...
void DisplayWordDP(String Word)
{
...
void DisplayWordSequence(byte sequence)
{
    ...
    case 4: // Display "NIGHT" "LIGHT"
        ...
        break;
    case 5: // Display "V 2.2.1" " LED " "TEST "  // Display software version number, 2.2.1
        if (wordSequenceStep == 1)
        {
            DisplayWord("V 221", 2000);
            DisplayWordDP("__11_");
        }
        else if (wordSequenceStep == 3)
        {
    ...
    case 9: // Display "TIME" "AND..."
        ...
        break;
    default: // Turn off word sequences. (Catches case 0.)
        wordSequence = 0;
        wordSequenceStep = 0;
    }
}
...
{
    // Display title of menu name after switching to new menu item.
...
void checkButtons(void)
{
    ...
    if (milliTemp >= NextButtonCheck) // Typically, go through this every 20 ms.
    {
        ...
        if (buttonMonitor)
        {
            // If any buttons have been down in last ButtonCheckInterval
            ...
            if ((buttonMonitor & a5_alarmSetBtn) && ((buttonStateLast & a5_alarmSetBtn) == 0))
            {
                ...
                if (alarmNow)
                {
                    // If alarm is going off, this button is the SNOOZE button.
        ...
        if (modeShowMenu && holdDebounce)
        {
            // Button behavior, when in Config menu mode:
            ...
            if (((buttonMonitor & a5_timeSetBtn) == 0) && (buttonStateLast & a5_timeSetBtn))
            {
                ...
                {
                    // If we are in a configuration menu
                    menuItem++;
        ...
        else
        {
            ...
            if (((buttonMonitor & a5_timeSetBtn) == 0) && (buttonStateLast & a5_timeSetBtn))
            {
                if (holdDebounce)
                {
                    ...
                    {
                        // If the time has just been adjusted, DO NOT change alarm status.
                        RedrawNow = 1;
                        UpdateEE = 1;
                        if (AlarmEnabled)
                        {
...
void EESaveSettings(void)
{
    ...
    if (milliTemp >= (LastButtonPress + 4000))
    {
        ...
        if (indicateEEPROMwritten)
        {
            // Blink LEDs off to indicate when we're writing to the EEPROM
            DisplayWord("     ", 100);
        }
...
void processSerialMessage()
{
    ...
    while (Serial.available() >= a5_COMM_MSG_LEN)
    {
        ...
        if (Serial.read() == a5_COMM_HEADER)
        {
            ...
            if (c == 'S')
            {
                if (c2 == 'T')
                {
                    ...
                    // Uncomment the following if you would like to be notified on time sync
            ...
            else if (c == 'B')
            {
                ...
                {                             // COMMAND: B0, Set Parameters
                    ...
                    {
                        // edit font character
                        // c2 : Indicates which ASCII character location to edit
                ...
                {
                    // Daisy chaining: With Bx, where x is less than 48 or x is less than 10:
                    if (c2 <= '9')
                    {
            ...
            else if (c == 'A')
            {
                ...
                {
                    // Daisy chaining: With Ax, where x is less than 48 or x is less than 10:
                    if (c2 <= '9')
                    {
                        // if we're here, c2 is <= '9', c2 != 0, and c2 != '0'.
        ...
        if (forceUpdateCopy)
        {
            if (DisplayModeLocalLast != 36)
            {
                ...
                FLWoffset = random(fiveLetterWordsMax); // Random word order!
                                                        // FLWoffset += 1;  // Alphabetical word order!
            }
